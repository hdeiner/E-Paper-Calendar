...
def draw_time_to_epaper(size, position_x, position_y, time, image, clock_face, logging):
    ...
    end_x = size*4/2 + math.sin(hour_radians)*((size*4/2)-100*4)
    end_y = size*4/2 - math.cos(hour_radians)*((size*4/2)-100*4)
    start_x = size*4/2 + math.sin(hour_radians)*(-20*4)
    start_y = size*4/2 - math.cos(hour_radians)*(-20*4)
    ...
    ImageDraw.Draw(space).line((start_x, start_y, (end_x, end_y)), fill=0, width=12*4)
    ...
    end_x = size*4/2 + math.sin(minute_radians)*((size*4/2)-30*4)
    end_y = size*4/2 - math.cos(minute_radians)*((size*4/2)-30*4)
    start_x = size*4/2 + math.sin(minute_radians)*(-20*4)
    start_y = size*4/2 - math.cos(minute_radians)*(-20*4)
